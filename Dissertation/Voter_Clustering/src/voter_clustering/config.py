...
# Mode A skips dao_cluster slices smaller than this (unless overridden by env).
def mode_a_min_sub_rows() -> int:
    override = os.environ.get("VOTER_CLUSTER_MIN_ROWS_MODE_A", "").strip()
    if override:
        return int(override)
    return max(30, (KMEANS_K_MAX + 1) * 5)
