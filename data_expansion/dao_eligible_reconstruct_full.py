...
def snapshot_graphql(query: str, variables: dict) -> dict:
    r = requests.post(SNAPSHOT_GRAPHQL, json={"query": query, "variables": variables}, timeout=60)
    r.raise_for_status()
    data = r.json()
    if "errors" in data:
...
def run_one_proposal(
    ...
    scores = batch_compute_scores(space=space, network=cfg.network, snapshot=snapshot_block, strategies=strategies,
                                  addresses=candidates, batch_size=cfg.score_batch_size, sleep_s=cfg.score_sleep_s)
...
def run_space(w3: Web3, cfg: RunConfig, space: str) -> List[dict]:
    ...
    if token_contract and cfg.use_etherscan_start_block and os.environ.get("ETHERSCAN_API_KEY"):
